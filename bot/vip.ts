...
async function notifyVipExpired(opts: {
  ...
  try {
    // Primeiro usa IDs explicitamente configurados. Se nenhum foi configurado,
    // envia automaticamente para o proprietário da aplicação/bot.
    const configuredIds = (process.env.VIP_EXPIRY_NOTIFY_USER_IDS ?? "")
      .split(",")
      .map(id => id.trim())
      .filter(Boolean);

    const recipientIds = new Set(configuredIds);
    if (!recipientIds.size) {
      const application = await client.application?.fetch().catch(() => null);
      const owner = application?.owner;
      if (owner && !("members" in owner)) recipientIds.add(owner.id);
    }

    if (!recipientIds.size) {
      logger.warn({ steamId, tier }, "VIP expiry notification skipped — no private recipient configured");
      return;
    }
    ...
    for (const userId of recipientIds) {
      try {
        ...
        logger.info({ userId, steamId, tier }, "VIP expiry notification sent privately");
      } catch (err) {
        logger.error({ err, userId, steamId, tier }, "Failed to send private VIP expiry notification");
      }
    }
  } catch (err) {
    logger.error({ err, steamId, tier }, "VIP expiry private notification failed");
  }
}
...
export async function revokeVip(opts: {
  ...
  // Expiração confirmada: avisa somente de forma privada, nunca no feed público.
