...
router.post("/wipe/vote",async(req,res)=>{
  try{
    ...
    const expectedWipeAt=Number(req.body?.expectedWipeAt);
    const result=await createMapVote(client,{createdBy:res.locals.admin.userId,date,maps,imageFiles,expectedWipeAt:Number.isFinite(expectedWipeAt)?expectedWipeAt:undefined});
    await auditWipe("MAP_VOTE_CREATED",{id:res.locals.admin.userId,name:res.locals.admin.username},`Votação ${result.id} criada pelo painel; wipe ${new Date(result.wipeAt).toISOString()}.`);res.status(201).json(result);
