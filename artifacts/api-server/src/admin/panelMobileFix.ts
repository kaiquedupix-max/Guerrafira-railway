...
.drawer input,.drawer select,.drawer textarea,.drawer .stateCard,.drawer .section,.drawer .actionGrid{max-width:100%!important;min-width:0!important;box-sizing:border-box!important}\
...
.drawer .btn{width:100%!important;min-height:48px!important}\
    ...
    for(const el of candidates){
      ...
      if(txt.includes('Portal')&&txt.includes('Ranking')&&txt.includes('Integridade')&&txt.includes('Controle')){
        ...
        if(matchMedia('(max-width:980px)').matches){
          const s=el.style;
          s.position='fixed';s.left='8px';s.right='8px';s.bottom='calc(8px + env(safe-area-inset-bottom))';s.top='auto';s.zIndex='1390';s.maxWidth='calc(100vw - 16px)';s.boxSizing='border-box';
        }
        break;
      }
    }
  ...
  function drawerState(){
    document.body.classList.toggle('gf-drawer-open',!!document.querySelector('.drawerBackdrop'));
  }
