...
export type AdminAlertKind = "anticheat" | "ban" | "kick" | "warn" | "unban" | "verify" | "vip" | "payment" | "system";
...
export function getWebPushPublicKey(): string | null { return process.env.WEB_PUSH_PUBLIC_KEY || null; }
...
export function pushAdminAlert(alert: Omit<AdminAlert, "id" | "createdAt">): AdminAlert {
  ...
  alerts.unshift(row); if (alerts.length > 250) alerts.length = 250; return row;
}

export function getAdminAlerts(limit = 100): AdminAlert[] { return alerts.slice(0, Math.max(1, Math.min(250, limit))); }
...
export async function saveAdminWebPushSubscription(userId: string, subscription: { endpoint: string; keys: { p256dh: string; auth: string } }): Promise<void> {
  ...
  await db.insert(adminWebPushSubscriptionsTable).values({ discordUserId: userId, endpoint: subscription.endpoint, p256dh: subscription.keys.p256dh, auth: subscription.keys.auth, enabled: true, updatedAt: new Date() })
    .onConflictDoUpdate({ target: adminWebPushSubscriptionsTable.endpoint, set: { discordUserId: userId, p256dh: subscription.keys.p256dh, auth: subscription.keys.auth, enabled: true, updatedAt: new Date() } });
}

export async function disableAdminWebPushSubscription(userId: string, endpoint?: string): Promise<void> {
  await ensureWebPushTable();
  if (endpoint) { await db.update(adminWebPushSubscriptionsTable).set({ enabled: false, updatedAt: new Date() }).where(eq(adminWebPushSubscriptionsTable.endpoint, endpoint)); return; }
...
export async function getAdminWebPushState(userId: string): Promise<boolean> {
  await ensureWebPushTable(); const rows = await db.select().from(adminWebPushSubscriptionsTable).where(eq(adminWebPushSubscriptionsTable.discordUserId, userId)); return rows.some(r => r.enabled);
}
...
async function notifyWebPush(row: AdminAlert): Promise<void> {
  ...
  const icon = row.kind === "anticheat" ? "🛡️" : row.kind === "payment" ? "💳" : row.severity === "critical" ? "🚨" : "";
  const payload = JSON.stringify({ title: `${icon} ${row.title}`.trim(), body: row.message, url: "/admin", tag: `gf-${row.kind}-${row.id}`, severity: row.severity });
  await Promise.all(subscribers.map(async sub => {
    try { await webpush.sendNotification({ endpoint: sub.endpoint, keys: { p256dh: sub.p256dh, auth: sub.auth } }, payload, { TTL: 3600, urgency: row.severity === "critical" ? "high" : "normal" }); }
    catch (err: any) {
      if (err?.statusCode === 404 || err?.statusCode === 410) await db.update(adminWebPushSubscriptionsTable).set({ enabled: false, updatedAt: new Date() }).where(eq(adminWebPushSubscriptionsTable.endpoint, sub.endpoint)).catch(() => {});
...
export async function notifySubscribedAdmins(alert: Omit<AdminAlert, "id" | "createdAt">): Promise<void> {
  const row = pushAdminAlert(alert); void notifyWebPush(row);
  ...
  await Promise.all(subscribers.map(async sub => { const user = await client.users.fetch(sub.discordUserId).catch(() => null); if (user) await user.send({ embeds: [embed] }).catch(err => logger.warn({ err, userId: sub.discordUserId }, "Failed to send admin DM notification")); }));
}
