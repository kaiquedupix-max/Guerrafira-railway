...
(function(){
  ...
  css.textContent='\
:root{--gf-gold:#f5c451;--gf-gold-soft:#f5c45122;--gf-surface:#100d09;--gf-surface-2:#17120c;--gf-border:#4b371f}\
html{scroll-behavior:smooth}body{background:radial-gradient(circle at 75% 0,#72451d55 0,transparent 34rem),#050403!important}\
...
.section,.card,.financeHero{border-color:var(--gf-border)!important;box-shadow:0 18px 50px #0003;background:linear-gradient(145deg,#151009,#0b0805)!important}.section{border-radius:22px!important}.card{border-radius:20px!important;transition:transform .2s,border-color .2s,box-shadow .2s}.card.click:hover{transform:translateY(-3px);border-color:#a87532!important;box-shadow:0 20px 45px #0006}.card strong{letter-spacing:-.035em}.sectionHead{padding:20px 22px!important}.body{padding:22px!important}\
...
.financeHero{padding:clamp(20px,4vw,34px)!important;border-radius:24px!important}.financeHeroTop h2{font-size:clamp(27px,4vw,40px)!important}.financeCards{gap:14px!important}.financeMetric{border-radius:19px!important;background:linear-gradient(145deg,#0b0805,#151009)!important}.financeMetric strong{font-size:clamp(24px,4vw,38px)!important}.financeTable tr:hover td{background:#1b130b}.btn{min-height:42px;transition:transform .15s,filter .15s}.btn:hover{filter:brightness(1.12);transform:translateY(-1px)}input,select,textarea{min-height:44px;border-radius:13px!important}.tableWrap{scrollbar-width:thin;scrollbar-color:#755026 #0b0805}\
      ...
      if(url.includes('/api/admin/finance/live')){
        ...
        setTimeout(function(){
          if(data.report&&data.report.status!=='ready'){
            ...
            if(data.report.status==='processing')setTimeout(function(){const finance=document.getElementById('finance'),days=document.getElementById('financeDays');if(finance&&finance.classList.contains('active')&&days)days.dispatchEvent(new Event('change'))},15000);
          }
        },0);
      }
    }catch{}
