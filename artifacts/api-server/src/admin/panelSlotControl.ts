...
(function(){
  ...
  function createPage(){
    ...
    sec.innerHTML='<div class="section"><div class="sectionHead"><div><h2>🎛️ Gerenciamento de Slots</h2><div class="subtitle">Defina como o Guerra Fria controla a capacidade do servidor.</div></div><span id="slotModeBadge" class="badge green">Carregando...</span></div><div class="body">'+
      ...
      '<div id="slotAutoFields"><div class="stateCard" style="margin-bottom:12px"><b>Modo automático</b><small>O bot controla os slots de acordo com população e fila, mas você escolhe exatamente o mínimo e o máximo permitidos.</small></div><div class="grid2"><div class="field"><label>Mínimo de slots</label><input id="slotMin" type="number" min="1" max="1000" step="1" inputmode="numeric"><div class="subtitle">O sistema nunca reduz abaixo deste limite.</div></div><div class="field"><label>Máximo de slots</label><input id="slotMax" type="number" min="1" max="1000" step="1" inputmode="numeric"><div class="subtitle">O sistema nunca aumenta acima deste limite.</div></div></div></div>'+
  ...
  async function save(){
    ...
    try{
      ...
      await load();if(!d.applied)toastLocal('Configuração salva, mas o RCON ainda não confirmou a alteração.',true);
    }catch(e){toastLocal(e.message,true)}finally{if(btn){btn.disabled=false;btn.textContent='Salvar configuração de slots'}}
  }
