...
router.get("/revenue-view", async (req, res) => {
  ...
  const bars = q.rows.map(r=>`<div style="flex:1;min-width:8px;background:#8b5cf6;height:${Math.max(3,Number(r.revenue||0)/max*180)}px" title="${r.day}: R$ ${Number(r.revenue).toFixed(2)}"></div>`).join('');
  ...
  const filters=[7,15,30,60,90].map(x=>`<a href="/api/finance/revenue-view?days=${x}" style="padding:9px 12px;border:1px solid #382a4b;border-radius:9px;color:white;text-decoration:none;${x===days?'background:#6d28d9':''}">${x} dias</a>`).join(' ');
