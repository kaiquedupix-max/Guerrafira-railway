...
export const data = new SlashCommandBuilder()
  .setName("mute")
  .setDescription("Muta um jogador do chat do servidor por um período")
  .addStringOption((opt) =>
    opt
      .setName("jogador")
  ...
  .addStringOption((opt) =>
    opt
      .setName("tempo")
      .setDescription("Tempo do mute. Ex.: 30m, 2h, 1d, 1d12h")
      .setRequired(true)
  )
  .addStringOption((opt) =>
    opt
...
export async function autocomplete(interaction: AutocompleteInteraction): Promise<void> {
  ...
  if (STEAM_ID_RE.test(focused) && !suggestions.some((s) => s.value === focused)) {
    suggestions.unshift({
      name: `SteamID ${focused}`.slice(0, 100),
      value: focused,
    });
  }
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    if (!DURATION_RE.test(duration) || !/\d/.test(duration)) {
      throw new ActionError("Tempo inválido. Use, por exemplo: 30m, 2h, 1d ou 1d12h.");
    }
    ...
    await executeRconRequired(
      `say <color=#FFB000>[JOGADOR MUTADO]</color> | <color=#FF8800>${safeChat(playerName, 80)}</color> foi mutado pelo administrador <color=#FF4444>${safeChat(interaction.user.tag, 60)}</color>. <color=#FFD166>Tempo:</color> <color=#FFFFFF>${safeChat(durationLabel(duration), 40)}</color> | <color=#FFD166>Motivo:</color> <color=#FFFFFF>${safeChat(reason, 140)}</color>`
    ).catch(() => null);

    await interaction.editReply(
      `✅ **${playerName}** foi mutado por **${durationLabel(duration)}**.\n📝 Motivo: ${reason}`
    );
  } catch (error) {
    await interaction.editReply(
      `❌ ${error instanceof ActionError ? error.message : "Falha interna ao mutar o jogador."}`
    );
  }
}
