...
type MapSelection =
  | { mode: "seed"; seed: number; size: number }
  | { mode: "link"; mapUrl: string };
...
function mapSelectionLabel(selection: MapSelection): string {
  return selection.mode === "seed"
    ? `Seed: \`${selection.seed}\` • Size: \`${selection.size}\``
    : "Mapa customizado via link `.map` do RustMaps";
}

function readMapSelection(interaction: ChatInputCommandInteraction): MapSelection {
  const mode = interaction.options.getString("modo", true) as MapMode;
  if (mode === "link") {
    const mapUrl = interaction.options.getString("mapa_url")?.trim();
    if (!mapUrl) throw new Error("No modo link, informe o campo mapa_url com o link .map do RustMaps.");
    return { mode, mapUrl };
  }
  const seed = interaction.options.getInteger("seed");
  const size = interaction.options.getInteger("size");
  if (seed === null || size === null) throw new Error("No modo seed, informe seed e size.");
  return { mode: "seed", seed, size };
}

function addMapSourceOptions(sub: any): any {
  return sub
    .addStringOption((opt: any)=>opt.setName("modo").setDescription("Como o novo mapa será carregado").setRequired(true).addChoices(
      { name: "Seed + Size", value: "seed" },
      { name: "Link .map (RustMaps Premium)", value: "link" },
    ))
    .addIntegerOption((opt: any)=>opt.setName("seed").setDescription("Seed do novo mapa (modo seed)").setMinValue(0).setMaxValue(2147483647))
    .addIntegerOption((opt: any)=>opt.setName("size").setDescription("Size do novo mapa (modo seed)").setMinValue(1000).setMaxValue(6000))
    .addStringOption((opt: any)=>opt.setName("mapa_url").setDescription("Link direto .map ou página do RustMaps (modo link)"));
}

async function sendManualWipeAnnouncement(
...
export const data = new SlashCommandBuilder()
  .setName("wipe")
  .setDescription("Diagnostica, planeja e executa wipes do servidor.")
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    if(sub==="travar"||sub==="destravar"){
      const state=await setWipeLock(sub==="destravar",`${interaction.user.tag} (${interaction.user.id})`);
      await auditWipe(state.unlocked?"WIPE_UNLOCKED":"WIPE_LOCKED",actor,`Trava alterada pelo Discord: ${state.unlocked?"liberado":"travado"}.`);
      await interaction.editReply({embeds:[new EmbedBuilder().setColor(state.unlocked?0x38c978:0xd64545).setTitle(state.unlocked?"🔓 Sistema de wipe destravado":"🔒 Sistema de wipe travado").setDescription(state.unlocked?"Execuções manuais e automáticas estão liberadas.":"Nenhum wipe poderá apagar arquivos enquanto a trava estiver ativa.").setTimestamp()]});
      return;
    }

    if(sub==="trava"){
      const state=await getWipeLockState();
      await interaction.editReply({embeds:[new EmbedBuilder().setColor(state.unlocked?0x38c978:0xd64545).setTitle(state.unlocked?"🔓 Wipe destravado":"🔒 Wipe travado").setDescription(state.updatedBy?`Última alteração: ${state.updatedBy}`:"Nenhuma alteração registrada.").setTimestamp()]});
      return;
    }

    if(sub==="diagnostico"){
      const d=await diagnoseHost();
      await auditWipe("WIPE_DIAGNOSTIC",actor,"Diagnóstico somente leitura executado pelo Discord.");
      ...
      const report=[
        ...
        `Variável de mapa customizado: ${d.levelUrlVariable??"não encontrada"}`,
        "",
      ...
      await interaction.editReply({embeds:[new EmbedBuilder().setColor(d.permissions.files.confirmed?0x38c978:0xd64545).setTitle("🛡️ Diagnóstico completo do wipe").setDescription("Nenhum save foi alterado. O diagnóstico também verifica suporte a mapas customizados por URL.").addFields(
        ...
        { name: "URL customizada", value: d.levelUrlVariable?`✅ ${d.levelUrlVariable}`:"❌ variável não encontrada", inline: false },
        ...
        { name: "Permissão procedural", value: `Seed (${d.seedVariable||"ausente"}): ${d.permissions.startup.seedEditable?"✅ editável":"❌ não editável"}\nSize (${d.sizeVariable||"ausente"}): ${d.permissions.startup.sizeEditable?"✅ editável":"❌ não editável"}`, inline: false },
      ...
      ).setTimestamp()],files:[{attachment:Buffer.from(report,"utf8"),name:`diagnostico-wipe-${Date.now()}.txt`} ]});
      return;
    }
    ...
    if(sub==="planejar"){
      const p=await buildWipePlan(kind,selection.mode==="link"?selection.mapUrl:undefined);
      await auditWipe("WIPE_PLAN",actor,`${kind}: ${p.files.length} arquivos; modo ${selection.mode}; nenhuma alteração.`);
      ...
      await interaction.editReply({embeds:[new EmbedBuilder().setColor(0xf4c45a).setTitle("📋 Plano de wipe — simulação").setDescription(`${preview}${p.files.length>20?`\n… e mais ${p.files.length-20}.`:""}`).addFields(
        ...
        { name: "Novo mapa", value: mapSelectionLabel(selection), inline: false },
    ...
    await sendManualWipeAnnouncement(interaction,"started",kind,selection);
    ...
    const result=await executeProceduralWipe(kind,selection.seed,selection.size,confirmation,actor);
    await sendManualWipeAnnouncement(interaction,"completed",kind,selection);
    await sendGameAnnouncement("GUERRA FRIA","Wipe concluido. Bom jogo!","#7CFC00").catch(()=>null);
    ...
    await interaction.editReply({embeds:[new EmbedBuilder().setColor(0xd64545).setTitle("🔒 Ação bloqueada").setDescription(error?.message||"Falha no sistema de wipe.").setTimestamp()]});
  }
}
