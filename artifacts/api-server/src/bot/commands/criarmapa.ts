...
function mapEmbeds(maps: MapOption[]) {
  return maps.map((m, i) => new EmbedBuilder()
    .setColor(i === 0 ? 0x95a5a6 : i === 1 ? 0x3498db : 0xf1c40f)
    .setTitle(`${i + 1}️⃣ ${m.name}`)
    .setDescription(`[🔗 Abrir página do mapa](${m.url})`)
    .setImage(m.image));
}
...
function rows(maps: MapOption[]) {
  return [new ActionRowBuilder<ButtonBuilder>().addComponents(...maps.map((m, i) =>
    new ButtonBuilder()
      .setCustomId(`mapvote:${i}`)
      .setLabel(`Votar no ${m.name}`)
      .setStyle(i === 0 ? ButtonStyle.Secondary : i === 1 ? ButtonStyle.Primary : ButtonStyle.Success)
  ))];
}
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const maps: MapOption[] = [1, 2, 3].map((n) => {
    ...
    if (!attachment.contentType?.startsWith("image/")) {
      throw new Error(`O arquivo enviado em imagem${n} não é uma imagem válida.`);
    }
    return {
      name: `Mapa ${n}`,
  ...
  const message = await channel.send({
    embeds: [headerEmbed(endsAt), ...mapEmbeds(maps)],
    components: rows(maps),
  });
...
export async function handleMapVote(interaction: ButtonInteraction): Promise<void> {
  ...
  await interaction.reply({
    content: `✅ Seu voto foi registrado em **${vote.maps[option].name}**. Você pode alterar o voto até o encerramento.`,
    ephemeral: true,
  });
}
...
async function finishVote(client: Client, messageId: string): Promise<void> {
  ...
  const counts = [0, 0, 0];
  for (const option of vote.votes.values()) counts[option]++;
  ...
  const result = vote.votes.size === 0
    ? "Nenhum voto foi registrado."
    : winners.length === 1
      ? `🏆 **${vote.maps[winners[0]].name}** venceu a votação!`
      : `🤝 Empate entre: **${winners.map(i => vote.maps[i].name).join(" • ")}**.`;
