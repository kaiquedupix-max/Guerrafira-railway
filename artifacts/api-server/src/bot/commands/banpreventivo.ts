...
export const data = new SlashCommandBuilder()
  ...
  .setDescription("Aplica banimento preventivo até verificação administrativa")
  .addStringOption((opt) =>
    opt
      .setName("jogador")
  ...
  .addStringOption((opt) =>
    opt
      .setName("motivo")
      .setDescription("Motivo do banimento preventivo")
      .setRequired(true)
  )
...
export async function autocomplete(interaction: AutocompleteInteraction): Promise<void> {
  ...
  if (STEAM_ID_RE.test(focused) && !suggestions.some((s) => s.value === focused)) {
    suggestions.unshift({
      name: `🛡️ PREVENTIVO • SteamID ${focused}`.slice(0, 100),
      value: focused,
    });
  }
