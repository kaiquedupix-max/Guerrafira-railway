...
router.get("/internal/season-email-repair-execute", async (req, res) => {
  const expected = String(process.env.SEASON_EMAIL_REPAIR_TOKEN || "");
  const supplied = String(req.query.token || "");
  if (!expected || supplied !== expected) return void res.status(404).json({ error: "not_found" });
  ...
  try {
    ...
    if (candidates.length !== 1) return void res.status(409).json({ error:"alexandre_ambiguous", candidates });
    ...
    return void res.json({ ok:true, repair_tag:REPAIR_TAG, before:Number(before?.rows?.[0]?.count||0), restored:updated?.rows?.length||0, alexandre:candidates[0], remaining_bad_email:after?.rows||[], backup_table:"season_email_repair_backup" });
  } catch (error) {
    return void res.status(500).json({ error:"repair_failed", detail:String(error) });
  }
});
