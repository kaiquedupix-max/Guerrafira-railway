...
router.get("/leaderboard", async (_req, res) => {
  try {
    ...
    const categories = {
      kills: top((r) => num(r.kills)), hs,
      wood: top((r) => num(r.woodGathered)), stone: top((r) => num(r.stoneGathered)),
      metal: top((r) => num(r.metalOreGathered)), sulfur: top((r) => num(r.sulfurOreGathered)),
      scrap: top((r) => num(r.scrapGathered)), gunpowder: top((r) => num(r.gunpowderCrafted)),
    ...
    const activePlayers = rows.filter((r) => num(r.kills) + num(r.resourcesGathered) + num(r.gunpowderCrafted) > 0).length;

    res.json({
      updatedAt: new Date().toISOString(),
      summary: { activePlayers, totalKills, totalFarm, leader: categories.kills[0]?.playerName ?? null },
      categories,
    });
  } catch {
...
<style>
:root{--bg:#08060d;--panel:#100d18;--panel2:#151020;--line:#2a2238;--text:#fbf9ff;--muted:#9d94ae;--purple:#8b5cf6;--purple2:#6d28d9;--yellow:#ffd84d;--gold:#ffd84d;--silver:#c9c4d3;--bronze:#d58a50}
*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;min-height:100vh;color:var(--text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:radial-gradient(circle at 50% -15%,#321765 0,#130d20 25%,#09070f 58%,#060509 100%)}
body:before{content:"";position:fixed;inset:0;pointer-events:none;background:linear-gradient(120deg,transparent 0 48%,#8b5cf605 49% 51%,transparent 52%)}
...
.hero{text-align:center;margin:58px auto 28px;max-width:840px}.eyebrow{color:var(--yellow);font-size:10px;font-weight:900;letter-spacing:.28em;text-transform:uppercase}.hero h2{font-size:clamp(38px,7vw,72px);line-height:.93;margin:14px 0 15px;letter-spacing:-.055em}.hero h2 span{background:linear-gradient(90deg,#a78bfa,#7c3aed 55%,#ffd84d);-webkit-background-clip:text;background-clip:text;color:transparent}.hero p{color:#a59ab6;font-size:14px;line-height:1.65;margin:auto;max-width:650px}
.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:34px 0 24px}.stat{background:linear-gradient(180deg,#151020,#0e0a15);border:1px solid #2c2340;border-radius:13px;padding:16px}.stat small{display:block;color:#80758f;text-transform:uppercase;letter-spacing:.14em;font-size:9px;font-weight:900}.stat strong{display:block;font-size:22px;margin-top:7px;letter-spacing:-.03em}.stat.leader strong{color:var(--yellow);font-size:16px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
<section class="hero"><div class="eyebrow">Leaderboard do wipe</div><h2>QUEM DOMINA O <span>GUERRA FRIA?</span></h2><p>Kills, precisão e farm registrados automaticamente durante o wipe. Escolha uma categoria e dispute seu lugar entre os melhores do servidor.</p></section>
<section class="stats"><div class="stat"><small>Jogadores ranqueados</small><strong id="sPlayers">—</strong></div><div class="stat"><small>Kills registradas</small><strong id="sKills">—</strong></div><div class="stat"><small>Farm registrado</small><strong id="sFarm">—</strong></div><div class="stat leader"><small>Líder de kills</small><strong id="sLeader">—</strong></div></section>
...
<script>
const meta={kills:{label:'Top Kills',metric:'Kills',sub:'Os jogadores mais letais do wipe.'},hs:{label:'Top HS%',metric:'HS%',sub:'Precisão de headshots — mínimo de 10 kills.'},wood:{label:'Madeira',metric:'Madeira',sub:'Quem mais farmou madeira no wipe.'},stone:{label:'Pedra',metric:'Pedra',sub:'Ranking de pedra coletada.'},metal:{label:'Metal',metric:'Metal',sub:'Minério de metal coletado.'},sulfur:{label:'Enxofre',metric:'Enxofre',sub:'Os maiores farmers de enxofre.'},scrap:{label:'Scrap',metric:'Scrap',sub:'Scrap registrado durante o wipe.'},gunpowder:{label:'Pólvora',metric:'Pólvora',sub:'Pólvora craftada pelos jogadores.'},farm:{label:'Farm Total',metric:'Recursos',sub:'Soma dos principais recursos coletados.'}};
let data=null,current='kills';const fmt=n=>new Intl.NumberFormat('pt-BR',{maximumFractionDigits:1,notation:n>=1000000?'compact':'standard',compactDisplay:'short'}).format(n);const steamMasked=id=>id?id.slice(0,7)+'••••••'+id.slice(-4):'';
function esc(v){return String(v??'').replace(/[&<>'\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#039;','"':'&quot;'}[c]))}
function renderSummary(){const s=data?.summary||{};document.getElementById('sPlayers').textContent=fmt(s.activePlayers||0);document.getElementById('sKills').textContent=fmt(s.totalKills||0);document.getElementById('sFarm').textContent=fmt(s.totalFarm||0);document.getElementById('sLeader').textContent=s.leader||'—'}
function render(){const list=data?.categories?.[current]||[],m=meta[current];document.getElementById('metricHead').textContent=m.metric;document.getElementById('metricBadge').textContent=m.metric;document.getElementById('sectionTitle').textContent=m.label;document.getElementById('sectionSub').textContent=m.sub;document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x.dataset.k===current));const medals=['🥇','🥈','🥉'];document.getElementById('podium').innerHTML=[0,1,2].map(i=>{const p=list[i];if(!p)return '';const cls=i===0?'first':i===1?'second':'third';return '<article class="pod '+cls+'"><div class="rank">#'+(i+1)+' DO SERVIDOR</div><div class="medal">'+medals[i]+'</div><div class="name">'+esc(p.playerName)+'</div><div class="value">'+fmt(p.value)+(current==='hs'?'%':'')+'</div></article>'}).join('');const root=document.getElementById('rows');if(!list.length){root.innerHTML='<div class="empty">Ainda não há dados suficientes nesta categoria.</div>';return}root.innerHTML=list.map((p,i)=>'<div class="row '+(i<3?'toprow':'')+'"><div class="pos '+(i<3?'top':'')+'">#'+(i+1)+'</div><div class="player"><div class="pname">'+esc(p.playerName)+'</div><div class="steam">'+steamMasked(p.steamId)+'</div></div><div class="score">'+fmt(p.value)+(current==='hs'?'%':'')+(p.secondary?'<div class="sub">'+esc(p.secondary)+'</div>':'')+'</div></div>').join('')}
