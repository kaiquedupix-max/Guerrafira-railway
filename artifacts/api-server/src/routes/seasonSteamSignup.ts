...
router.post("/season/:number/inscricao", async (req, res, next) => {
  ...
  try {
    await db.execute(sql`INSERT INTO season_registrations (season_number,discord_id,discord_name,steam_id,mode,status,accepted_rules_at,updated_at)
      VALUES (${season},${session.userId},${session.username},${steamId},'beta_free','active',now(),now())
      ON CONFLICT (season_number,discord_id) DO UPDATE SET discord_name=EXCLUDED.discord_name,steam_id=EXCLUDED.steam_id,status='active',accepted_rules_at=now(),updated_at=now()`);
